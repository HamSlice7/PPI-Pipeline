...
for row in range(df_validation_sampled.shape[0]):
    peptidase = df_validation_sampled['Peptidase'][row]
    peptidase_family = df_validation_sampled['Peptidase Family'][row]
    inhibitor_family = random.choice([fam for fam in inhibitor_families if fam != peptidase_family])
    inhibitor = df_validation_sampled[df_validation_sampled['Inhibitor Family'] == inhibitor_family]['Inhibitor'].sample(n=1, random_state = seed).values[0]
    random_pairs.append({'Peptidase': peptidase, 'Peptidase Family': peptidase_family, 'Inhibitor': inhibitor, 'Inhibitor Family': inhibitor_family})
